...
class ChannelTDataset(Dataset):
    ...
    def __init__(self, config_filename, simulator_dir, numUEs, transform=None):
        """
        Arguments:
        ...
        cache_name = "{}_{}kmH_{}ue_{}s_{}ms_{}rx_{}tx.pt".format(config_filesplit[0], self.ueSpeed, self.numUEs, self.numSteps, self.stepSize, self.ueAntPerPanel, self.bsAntPerPanel)
        ...
        chanLen = lib.getComChanLength(b)
        lib.chanfexp.restype = c_char_p
        self.h_buffer = np.zeros((self.numUEs, self.numSteps, self.ueAntPerPanel, self.bsAntPerPanel, chanLen * 2), dtype=np.float32)
        self.t_buffer = np.zeros((self.numUEs, chanLen), dtype=np.float32)
        for i in range(numOfDrops):
            ChanFFloats = c_float * (chanLen * 2) * self.bsAntPerPanel * self.ueAntPerPanel *  self.numSteps * numUEsPerDrop
            ChanFBuffer = ChanFFloats()
            ChanLFloats = c_float * chanLen * numUEsPerDrop
            ChanLBuffer = ChanLFloats()
            lib.chanfexp(b, -1, ChanFBuffer, ChanLBuffer)
            h_buffer = np.ctypeslib.as_array(ChanFBuffer, (numUEsPerDrop, self.numSteps, self.ueAntPerPanel, self.bsAntPerPanel, chanLen * 2))
            t_buffer = np.ctypeslib.as_array(ChanLBuffer, (numUEsPerDrop, chanLen))
